<script>
  ...
  onMount(async () => {
    ...
    let MAGNIFY = OPTS.mag || OPTS.magnify || 3;
    ...
    let CurScore = {};
    let DEFAULTMAXBARS = 199 * 4 + 1; // 24 bars by default
    let DEFAULTTEMPO = 100;
    let CurMaxBars = DEFAULTMAXBARS;
    let Mario = null; // Mamma Mia!
    let AnimeID = 0; // ID for cancel animation
    let PsedoSheet = null; // CSSRules for manipulating pseudo elements
    ...
    MarioClass.prototype.draw = function () {
      ...
      if (this.isJumping) {
        ...
        if (this.x == 120) {
          ...
          if (this.scroll != 16) {
            y -= this.jump(
              this.scroll > 16 ? this.scroll - 16 : this.scroll + 16,
            );
    ...
    for (var i = 1; i < 21; i++) {
      var tmp = '0';
      tmp += i.toString();
      var file = 'MarioSequencer/wav/sound' + tmp.substr(-2) + '.wav';
    ...
    var mi = new Image();
    mi.src = 'MarioSequencer/image/mat.png';
    mi.onload = function () {
      L1C.drawImage(mi, 0, 0, mi.width * MAGNIFY, mi.height * MAGNIFY);
    };

    // Prepare Characters
    let char_sheet = new Image();
    char_sheet.src = 'MarioSequencer/image/character_sheet.png';

    // Prepare the Bomb!
    let BOMBS = [];
    let bombimg = new Image();
    bombimg.src = 'MarioSequencer/image/bomb.png';
    let bombTimer = new easyTimer(150, drawBomb);
    ...
    let GClef = new Image();
    GClef.src = 'MarioSequencer/image/G_Clef.png';

    // Prepare the numbers
    let numimg = new Image();
    numimg.src = 'MarioSequencer/image/numbers.png';
    let NUMBERS = sliceImage(numimg, 5, 7);

    // Prepare the Mario images
    let marioimg = new Image();
    marioimg.src = 'MarioSequencer/image/Mario.png';

    let sweatimg = new Image();
    sweatimg.src = 'MarioSequencer/image/mario_sweat.png';

    // Prepare the Play button
    let playbtnimg = new Image();
    playbtnimg.src = 'MarioSequencer/image/play_button.png';

    // Prepare the Stop button
    let stopbtnimg = new Image();
    stopbtnimg.src = 'MarioSequencer/image/stop_button.png';

    // Prepare the CLEAR button
    let clearimg = new Image();
    clearimg.src = 'MarioSequencer/image/clear_button.png';

    // Prepare tempo range slider thumb image
    let thumbimg = new Image();
    thumbimg.src = 'MarioSequencer/image/slider_thumb.png';

    // Prepare beat button
    let beatimg = new Image();
    beatimg.src = 'MarioSequencer/image/beat_button.png';

    // Prepare Song buttons
    let songimg = new Image();
    songimg.src = 'MarioSequencer/image/song_buttons.png';

    // Prepare End Mark
    let endimg = new Image();
    endimg.src = 'MarioSequencer/image/end_mark.png';

    // Prepare Semitone
    let semitoneimg = new Image();
    semitoneimg.src = 'MarioSequencer/image/semitone.png';

    // Prepare the repeat marks
    let repeatimg = new Image();
    repeatimg.src = 'MarioSequencer/image/repeat_head.png';
    ...
    function closing() {
      ...
      var b = document.getElementById(
        CurScore.beats == 3 ? '3beats' : '4beats',
      );
    ...
    function addMSQ(text) {
      ...
      lines.forEach(function (line, i) {
        ...
        if (i < keyword.length && k !== keyword[i]) {
          throw new Error(
            'Line ' + i + " must start with '" + keyword[i] + "'",
          );
        }
        this[k] = v;
      }, values);
    ...
    function onload() {
      ...
      endMarkTimer = new easyTimer(150, function (self) {
        ...
        SCREEN.style.cursor =
          'url(' +
          self.images[self.currentFrame].src +
          ')' +
          7 * MAGNIFY +
          ' ' +
          7 * MAGNIFY +
      ...
      // Prepare Repeat (global!)
      let RepeatMarks = sliceImage(repeatimg, 13, 62);
      EndMark = RepeatMarks[2];
      ...
      let Semitones = sliceImage(semitoneimg, 5, 12);
          ...
          if (OPTS['url'] != undefined) {
            ...
            new Promise(function (resolve, reject) {
              ...
              .catch(function (err) {
                ...
                console.error(
                  'Downloading File: ' + url + ' failed :' + err.stack,
                );
              });
    ...
    function playListener(e) {
      ...
      ['toLeft', 'toRight', 'scroll', 'clear', 'frog', 'beak', '1up'].map(
        function (id) {
...
<main>
  ...
  >
    ...
    <image
      id="spinner"
      src="/MarioSequencer/image/loading_transparent_.gif"
      width="46px"
      style="position: absolute; top:35%; left:47%; z-index:4;"></image>
  </div>
  <!-- <center>
<select id="magnify">
