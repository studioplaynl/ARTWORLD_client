<script>
    ...
    const unsubscribe = itemsBar.subscribe(async (value) => {
        console.log(value);
        HistoryTracker = (await import("../game/class/HistoryTracker.js"))
            .default;
        ...
        if (user_house_url == undefined) {
            user_house_url = await getObject(
                "home",
                $Profile.meta.Azc,
                $Profile.id
            );
            user_house_url = await convertImage(
                user_house_url.value.url,
                "50",
                "50"
            );
        }
