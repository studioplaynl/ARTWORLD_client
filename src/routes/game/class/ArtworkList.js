...
class ArtworkList {
  ...
  placeHeartButton(scene, x, y, keyImgUrl, mediaObject) {
    ...
    const currentHeart = scene.add.image(x, y + (artFrame.height / 2), 'heart').setOrigin(1, 0).setScale(0.7)
      .setInteractive()
  ...
  async heartButtonToggle(scene, mediaObject, button) {
    ...
    const parsedMediaOject = {
      user_id: mediaObject.user_id, collection: mediaObject.collection, key: mediaObject.key, version: mediaObject.value.version, url: mediaObject.value.url, previewURl: mediaObject.value.previewURl,
    };
