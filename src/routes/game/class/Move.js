...
class Move {
  ...
  movingAnimation(scene, animation) {
    const splitCurrentMovingAnimation = scene.player.anims.currentAnim.key.split('_')
    //console.log("splitMovingKey[0]", splitMovingKey[0])
    if (animation === 'moving' && splitCurrentMovingAnimation[0] === 'stop') {
      scene.player.anims.play(scene.playerMovingKey, true);
      scene.playerShadow.anims.play(scene.playerMovingKey, true);
    }

    if (animation === 'stop' && splitCurrentMovingAnimation[0] === 'moving') {
