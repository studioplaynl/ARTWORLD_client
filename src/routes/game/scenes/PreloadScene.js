...
export default class PreloadScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.svg('ui_magnifier_plus', './assets/SHB/svg/AW-icon-plus.svg', {
      scale: 3,
    });
    this.load.svg('ui_eye', './assets/SHB/svg/AW-icon-reset.svg', { scale: 4 });

    this.load.svg('back_button', './assets/SHB/svg/AW-icon-previous.svg', {
      scale: 0.5,
    });
    ...
    this.load.svg('heart', './assets/SHB/svg/AW-icon-heart-full-red.svg', {
      scale: 0.7,
    });
    this.load.svg('heart_empty', './assets/SHB/svg/AW-icon-heart.svg', {
      scale: 0.7,
    });

    this.load.svg('enter_home', './assets/SHB/svg/AW-icon-enter-space.svg', {
      scale: 0.5,
    });
    this.load.svg('save_home', './assets/SHB/svg/AW-icon-save.svg', {
      scale: 0.5,
    });
    this.load.svg('addressbook', './assets/SHB/svg/AW-icon-addressbook.svg', {
      scale: 0.7,
    });
    this.load.svg('close', './assets/SHB/svg/AW-icon-cross.svg', {
      scale: 0.6,
    });
    this.load.svg('delete', './assets/SHB/svg/AW-icon-trash.svg', {
      scale: 0.6,
    });
  ...
  createHitAreaLocations() {
    // create a hitArea for locations, as an image with key 'enterButtonHitArea', 128x128pix
    const width = 128;
    const enterButtonHitArea = this.add
      .rectangle(width / 2, width / 2, width, width, 0x6666ff)
  ...
  createWhiteSquare(postFix) {
    ...
    // create renderTexture to place the dot on
    const artFrameRendertexture = this.add
      .renderTexture(0, 0, postFix, postFix)
      .setVisible(false);
